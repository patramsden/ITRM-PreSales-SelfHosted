...
const CATEGORIES: Category[] = [
  ...
  { id: 'proposals',  label: 'Project Proposals',        icon: '📄' },
  { id: 'support',    label: 'Support Contracts',        icon: '🛡️' },
  ...
  { id: 'integrations', label: 'Integrations & AI',     icon: '🤖' },
];
...
const ARTICLES: Article[] = [
...

  {
    ...
    content: [
      ...
      { type: 'table', headers: ['Field', 'Description'], rows: [
        ...
        ['Unit',             'Days (default) or hours. Click the "days" / "hrs" toggle on a task row to switch.'],
      ...
      { type: 'h2', text: 'Unit constraints' },
      { type: 'p', text: 'Effort quantities must follow the unit\'s increment rules: hours must be entered as whole numbers (1, 2, 3…); days can be in half-day increments (0.5, 1, 1.5, 2…). If you type a non-conforming value, it snaps to the nearest valid increment when you move to the next field.' },
...

  {
    ...
    category: 'proposals',
    content: [
      { type: 'p', text: 'The Statement of Work (SoW) tab contains the detailed project scope document that accompanies the proposal. It uses a full rich text editor so you can format content professionally.' },
      { type: 'h2', text: 'Rich text editor toolbar' },
      { type: 'p', text: 'The SoW editor has a full formatting toolbar above the writing area:' },
      { type: 'table', headers: ['Button', 'Shortcut', 'What it does'], rows: [
        ['Undo / Redo',           'Ctrl+Z / Ctrl+Y', 'Step backwards and forwards through changes'],
        ['H1 / H2 / H3',          '—',               'Apply a section heading at three levels'],
        ['Bold',                  'Ctrl+B',           'Bold the selected text'],
        ['Italic',                'Ctrl+I',           'Italicise the selected text'],
        ['Underline',             'Ctrl+U',           'Underline the selected text'],
        ['Bullet list',           '—',               'Start or convert to an unordered bullet list'],
        ['Numbered list',         '—',               'Start or convert to an ordered numbered list'],
        ['Align L / C / R',       '—',               'Set paragraph alignment'],
        ['Link',                  '—',               'Insert a hyperlink (you will be prompted for the URL)'],
        ['Remove link',           '—',               'Appears when cursor is inside a link — removes it'],
      ...
      { type: 'note', text: 'The AI generates a starting draft. Always review and edit the output before sending it to a client.' },
      { type: 'h2', text: 'Inserting clauses' },
      { type: 'p', text: 'Click "Insert Clause" to open the Clause Library picker. Search or browse by category, preview the clause on the right, then click Insert to append it to the SoW.' },
      { type: 'h2', text: 'Saving' },
      { type: 'p', text: 'Click Save SoW to persist your changes. Content is stored as HTML so formatting is preserved exactly as typed.' },
      { type: 'tip', text: 'Use Regenerate to refresh the AI content. It replaces the editor content, so save any manual edits you want to keep first.' },
    ],
  },
  ...
  {
    id: 'discount',
    title: 'Discount Approval & Justification',
    category: 'reviews',
    content: [
      { type: 'p', text: 'The Discount tab manages consultancy discounts — fixed £ reductions or percentage discounts applied to the consultancy sell total. Any discount also triggers the TRB review requirement.' },
      { type: 'h2', text: 'Applying a discount' },
      { type: 'steps', items: [
        'Go to the Discount tab.',
        'Choose Percentage (%) or Monetary (£) discount type.',
        'Enter the discount amount.',
        'You must enter a written justification in the Discount Justification field — this is required before export.',
      ]},
      { type: 'warn', text: 'The Discount Justification field is mandatory whenever a discount is applied. The proposal cannot be exported until you fill it in. This ensures a clear audit trail of why pricing was reduced.' },
      { type: 'h2', text: 'Discount Approval (markup floor)' },
      { type: 'p', text: 'A separate Discount Approval is triggered automatically in the Approvals tab when the proposal\'s Markup % drops below the company floor (default 10%). It ensures below-floor pricing is reviewed before the proposal goes out.' },
      { type: 'steps', items: [
...

  {
    ...
    content: [
      ...
      { type: 'table', headers: ['Blocker', 'When it applies', 'How to resolve'], rows: [
        ...
        ['Discount justification missing', 'A discount is applied but the justification field is empty', 'Fill in the justification on the Discount tab'],
...

  {
    ...
    content: [
      ...
      { type: 'steps', items: [
        ...
        'Click New Clause.',
        'Give it a title (e.g. "Standard Project Assumptions") and a category.',
        'Use the rich text editor to write and format the clause content.',
        'Click Add Clause to save.',
      ]},
      { type: 'h2', text: 'Formatting clauses' },
      { type: 'p', text: 'Clause content uses the full rich text editor — you can use headings, bold, italic, bullet and numbered lists, links, and text alignment. Formatted clauses are inserted into the SoW preserving all formatting.' },
...

  {
    ...
    content: [
      ...
      { type: 'p', text: 'The Client field on the Summary tab searches Autotask companies as you type. Select a result and a blue "CRM" badge appears — this means the proposal is linked to the Autotask company ID, which unlocks contact lookup, address auto-fill, and ticket display.' },
      { type: 'h2', text: 'Account Manager auto-population' },
      { type: 'p', text: 'When you select a company from Autotask, the app immediately looks up the account manager resource assigned to that company and fills the Account Manager field. Use the ↺ button to retry if it didn\'t fill in.' },
      { type: 'h2', text: 'Client Address auto-fill' },
      { type: 'p', text: 'Selecting a company also fetches the company\'s registered address from Autotask (address1, address2, city, postal code, country) and populates the Client Address field on the Summary tab. You can edit it after import.' },
      { type: 'h2', text: 'Client Contact with email' },
      { type: 'p', text: 'The Client Contact field shows a dropdown of active contacts for the linked company. Selecting a contact also auto-fills the Contact Email field with the email address held in Autotask.' },
      { type: 'h2', text: 'Open tickets panel' },
      { type: 'p', text: 'When a company is linked, a right-hand panel shows recent open tickets from Autotask for that company across the Account Management, Pre-Sales, and Post-Sale queues. Click a ticket title to open it in Autotask. Use "Use as Ref" to link a ticket number to the proposal.' },
      { type: 'h2', text: 'Autotask project creation' },
  ...
  {
    id: 'support-overview',
    title: 'Support Proposal Overview',
    category: 'support',
    content: [
      { type: 'p', text: 'Support proposals are a distinct proposal type for quoting IT Managed Service Agreements (MSAs). Unlike project proposals, they are priced on a recurring per-user monthly model.' },
      { type: 'h2', text: 'Creating a support proposal' },
      { type: 'steps', items: [
        'From the Proposals list, click New Proposal → New Support Proposal.',
        'The 3-step wizard walks you through Contract Details, Users & Add-ons, and a Review screen.',
        'The new proposal opens with its own set of tabs: Summary, Support Contract, Totals, Document, Billing, Approvals, Comments.',
      ]},
      { type: 'h2', text: 'How support proposals differ from project proposals' },
      { type: 'table', headers: ['Aspect', 'Project Proposal', 'Support Proposal'], rows: [
        ['Pricing model',   'One-off hardware, software + consultancy days', 'Monthly per-user recurring (MRR)'],
        ['Key metric',      'Grand Total + GP%',                             'MRR / ARR / TCV'],
        ['Contract tab',    'Parts + Consultancy tabs',                      'Support Contract tab'],
        ['Document',        'PDF proposal + SoW',                           'IT Managed Service Agreement document'],
        ['Billing',         'Milestones on a project total',                 'Recurring billing cycle'],
      ]},
    ],
  },
  ...
  {
    id: 'support-wizard',
    title: 'Support Proposal Wizard',
    category: 'support',
    content: [
      { type: 'p', text: 'The wizard creates a new support proposal in three steps.' },
      { type: 'h2', text: 'Step 1 — Contract Details' },
      { type: 'ul', items: [
        'Client — search Autotask or type free text. Selecting from Autotask auto-fetches the account manager and company address.',
        'Support Hours — choose Mon–Fri 9–5 (£40/seat), Mon–Fri 8–6 (£50/seat), or Mon–Fri 7–7 (£60/seat). These are defaults; the price can be overridden.',
        'Contract Term — 1 year (no discount), 3 years (up to 5% term discount), or 5 years (up to 10% term discount).',
        'Billing Cycle — Monthly, Quarterly, or Annually.',
        'Document & Contact Details (optional) — MSP contact name, email, phone, mobile for the cover page; client contact name (CRM picker when company is linked); client address (auto-filled from CRM); contract start date; site.',
      ]},
      { type: 'h2', text: 'Step 2 — Users & Add-ons' },
      { type: 'ul', items: [
        'Full-time users — billed at the full per-seat price.',
        'Part-time users — billed at 50% of the per-seat price.',
        'Onboarding — automatically calculated as 1 month\'s base cost (no term discount, no add-ons). Added to Year 1.',
        'Optional Add-ons — catalog items tagged as "Support Add-on" can be selected here. Each shows its per-seat or flat monthly price.',
      ]},
      { type: 'h2', text: 'Step 3 — Review' },
      { type: 'p', text: 'Shows a full contract summary and the key financial figures: MRR, ARR, TCV, and onboarding. Click Create Proposal to save.' },
    ],
  },

  {
    id: 'support-contract-tab',
    title: 'Support Contract Tab',
    category: 'support',
    content: [
      { type: 'p', text: 'After creation, the Support Contract tab lets you edit all aspects of the contract without re-running the wizard.' },
      { type: 'h2', text: 'Editable fields' },
      { type: 'p', text: 'All the fields from the wizard are accessible: support hours, pricing, seats, part-time seats, term, billing cycle, add-ons, inclusions, and exclusions.' },
      { type: 'h2', text: 'Inclusions and exclusions' },
      { type: 'p', text: 'The Scope of Services table in the contract document is driven by the inclusions (✓) and exclusions (✗) you define here. Default inclusions are: unlimited remote helpdesk, onsite callout, preventative maintenance, quarterly service review, dedicated account management, vendor management, patch management, and server/network monitoring.' },
    ],
  },

  {
    id: 'support-totals',
    title: 'Support Contract Totals',
    category: 'support',
    content: [
      { type: 'p', text: 'The Totals tab for support proposals shows the commercial summary for the MSA.' },
      { type: 'h2', text: 'Key metrics' },
      { type: 'table', headers: ['Metric', 'Description'], rows: [
        ['MRR',             'Monthly Recurring Revenue — includes base seats (after term discount), part-time seats, and all add-ons.'],
        ['ARR',             'Annual Recurring Revenue = MRR × 12.'],
        ['TCV',             'Total Contract Value = MRR × term months (12, 36, or 60).'],
        ['Onboarding',      'One-time setup fee = 1 month\'s base cost (no discount). Added to Year 1 total.'],
        ['Year 1 total',    'ARR + onboarding. Higher than subsequent years.'],
        ['Payment schedule','Shows quarterly or annual invoice amounts based on billing cycle.'],
      ]},
    ],
  },
  ...
  {
    id: 'support-document',
    title: 'Support Agreement Document',
    category: 'support',
    content: [
      { type: 'p', text: 'The Document tab renders a full IT Managed Service Agreement in a printable page format, mirroring a professional 13-section MSA document.' },
      { type: 'h2', text: 'Document structure' },
      { type: 'ul', items: [
        '§1 Confidential Information & Contact',
        ...
        '§8 Contractual Requirements (including SLA table)',
        'Custom sections — any sections you add (see below)',
        '§9 Schedule 1 — Scope of Services, Supported Users, Commercial Overview',
        '§10 Contractual Terms & financial summary',
        '§11 Service Contract',
        '§12 Authorised Signatures',
      ]},
      { type: 'h2', text: 'Boilerplate sections (§2–§8)' },
      { type: 'p', text: 'Sections §2 through §8 use company-wide boilerplate text stored in Settings → Support Document. Admins can edit the text by hovering over a section and clicking the pencil icon. Changes apply globally to all new and existing documents.' },
      { type: 'h2', text: 'Section images' },
      { type: 'p', text: 'Each boilerplate section (and each custom section) can have an image attached — useful for partner logos on the Certifications page. Click the pencil to edit a section, then click "Add image" to upload. The image renders below the section text in both the preview and the PDF.' },
      { type: 'h2', text: 'Rich text in sections' },
      { type: 'p', text: 'All boilerplate and custom section editors use the full rich text editor with headings, bullet lists, bold, italic, and links. Formatting is preserved in the on-screen preview. The PDF export converts rich text to structured plain text (headings → larger text, bullets → • prefix).' },
      { type: 'h2', text: 'Custom sections' },
      { type: 'p', text: 'Click "Add Section" in the toolbar to insert a custom page between §8 and Schedule 1. Each section has a title, rich text body, and optional image. Use the ▲▼ arrows to reorder, the pencil to edit, and the trash icon to delete.' },
      { type: 'h2', text: 'SLA table (§8.2)' },
      { type: 'p', text: 'The SLA hours for P1 (Critical), P2 (Standard), and P3 (Service Request) priorities are editable directly in the table. Click the number field and type the new value.' },
      { type: 'h2', text: 'Exporting to PDF' },
      { type: 'steps', items: [
        'Click "Export PDF" in the toolbar to generate a professionally formatted vector PDF.',
        'The PDF is generated client-side using the same library as project proposal PDFs.',
        'Click "Print" to open the browser print dialog for printing or saving as a browser-rendered PDF.',
      ]},
      { type: 'note', text: '"Export PDF" produces a cleaner result than "Print" for sharing with clients. Use "Print" only if you need to include browser-rendered content.' },
    ],
  },

  {
    id: 'support-catalog-addons',
    title: 'Support Add-ons in the Catalog',
    category: 'support',
    content: [
      { type: 'p', text: 'Catalog items can be tagged as available as support add-ons, making them selectable in the support proposal wizard and contract tab.' },
      { type: 'h2', text: 'Tagging a catalog item as a support add-on' },
      { type: 'steps', items: [
        'Go to Catalog and open any item for editing.',
        'Tick "Available as Support Add-on".',
        'Set the pricing type: Per seat/month (price is multiplied by the number of full-time seats) or Flat/month (a fixed monthly cost regardless of seat count).',
        'Save the item.',
      ]},
      { type: 'p', text: 'Tagged items appear in the Add-ons picker in the wizard (Step 2) and on the Support Contract tab. Support add-on items are shown with a purple "Support Add-on" badge in the Catalog list.' },
    ],
  },
...

  {
    ...
    content: [
      { type: 'p', text: 'All major content fields in the app use a rich text editor with a Word-like toolbar. This replaces the previous plain-text areas.' },
      { type: 'h2', text: 'Where rich text is used' },
      { type: 'ul', items: [
        'Statement of Work (full toolbar with headings, alignment, links)',
        'Clause Library content',
        'Proposal narrative fields: Objectives, Business Requirements, Justification, Constraints, Assumptions, Notes (compact toolbar)',
        'Support document boilerplate sections and custom sections',
        'Template descriptions',
      ]},
      { type: 'h2', text: 'Toolbar reference' },
      { type: 'table', headers: ['Button', 'Keyboard', 'Action'], rows: [
        ['Undo',        'Ctrl+Z', 'Undo last change'],
        ['Redo',        'Ctrl+Y', 'Redo undone change'],
        ['H1',          '—',      'Heading level 1 (large)'],
        ['H2',          '—',      'Heading level 2 (medium)'],
        ['H3',          '—',      'Heading level 3 (small)'],
        ...
        ['Align',       '—',      'Left / Centre / Right alignment'],
        ['Link',        '—',      'Insert hyperlink (prompted for URL)'],
        ['Clear',       '—',      'Remove all formatting from selection'],
      ]},
      { type: 'h2', text: 'Backward compatibility' },
      { type: 'p', text: 'Existing content created before the rich text editor was introduced (stored as plain text) is automatically detected and displayed correctly. It continues to work without any migration.' },
      { type: 'h2', text: 'PDF output' },
      { type: 'p', text: 'Rich text content is preserved in the PDF export. Headings map to appropriately sized text, bullet lists render with bullet symbols, and bold/italic text is styled accordingly.' },
    ],
  },

  {
    id: 'mcp-copilot',
    title: 'Microsoft Copilot Integration (MCP)',
    category: 'integrations',
    content: [
      { type: 'p', text: 'The self-hosted server exposes an MCP (Model Context Protocol) endpoint at /mcp that lets Microsoft Copilot (and other AI assistants) query your live proposal and contract data in natural language.' },
      { type: 'h2', text: 'What you can ask Copilot' },
      { type: 'ul', items: [
        '"What proposals are currently in progress?"',
        '"What\'s our total active pipeline value?"',
        '"Show me all proposals for Acme Corp"',
        '"What\'s our MRR from active support contracts?"',
        '"What changed this week?"',
        '"What day rates do we charge for senior consultants?"',
        '"Find any proposals mentioning Azure migration"',
      ]},
      { type: 'h2', text: 'Available tools (what Copilot can call)' },
      { type: 'table', headers: ['Tool', 'Description'], rows: [
        ['list_proposals',             'List proposals with optional status, type, or client filter'],
        ['get_proposal',               'Full detail for a single proposal by ID'],
        ['search_proposals',           'Relevance-ranked text search across project name and client'],
        ['get_pipeline_summary',       'Counts and values by stage; win rate'],
        ['list_support_contracts',     'All MSA proposals with MRR, ARR, and TCV'],
        ['get_financial_summary',      'Whole-book KPIs: pipeline value, won revenue, active MRR'],
        ['list_proposals_for_client',  'All proposals for a named client'],
        ['get_catalog_items',          'Search the product/service catalog'],
        ['get_rate_cards',             'Consultancy roles, sell rates, and margins'],
        ['get_recent_activity',        'Most recently modified proposals'],
      ]},
      { type: 'h2', text: 'Setting up the Copilot connector' },
      { type: 'steps', items: [
        'In the app: Settings → System → Generate Service Key. Copy the key shown.',
        'In Copilot Studio: open your Copilot → Actions → Add an action → New connector.',
        'Set the URL to https://<your-server>/mcp.',
        'Set authentication to API Key, header name "Authorization", value "Bearer <your-key>".',
        'Save and publish the connector.',
        'Test by asking Copilot: "What proposals are in progress?"',
      ]},
      { type: 'h2', text: 'Authentication' },
      { type: 'p', text: 'All MCP requests require a valid service API key passed as a Bearer token. The key is a long-lived credential — treat it like a password. You can revoke and regenerate it in Settings → System at any time.' },
      { type: 'h2', text: 'Local testing with Claude Desktop' },
      { type: 'p', text: 'To test the MCP server with Claude Desktop, add the following to ~/.claude/claude_desktop_config.json:' },
      { type: 'note', text: '{ "mcpServers": { "itrm-presales": { "url": "http://localhost:3001/mcp", "headers": { "Authorization": "Bearer <your-key>" } } } }' },
      { type: 'h2', text: 'Health check' },
      { type: 'p', text: 'GET /mcp/health returns the server status and active session count. This endpoint does not require authentication and can be used as a liveness probe.' },
    ],
  },

  {
    id: 'proposal-stages',
    title: 'Proposal Status & Progress Bar',
    category: 'proposals',
    content: [
      { type: 'p', text: 'Every proposal moves through a defined set of stages from creation to close. The current stage is shown in a progress bar at the top of the proposal workspace.' },
      { type: 'h2', text: 'Stage definitions' },
      { type: 'table', headers: ['Stage', 'Description'], rows: [
        ['New',               'Proposal created but not yet being actively worked on.'],
        ['In Progress',       'Active proposal being built and costed.'],
        ['Waiting Approval',  'Submitted for internal commercial or technical review.'],
        ['Approved',          'Internal review complete — ready to send.'],
        ['Sent to Customer',  'Proposal has been shared with the client.'],
        ['Won',               'Client accepted — create Autotask project and hand over to delivery.'],
        ['Lost',              'Client declined or chose a competitor.'],
      ]},
      { type: 'h2', text: 'Changing status' },
      { type: 'p', text: 'Click any stage in the progress bar to jump to it, or change the Status field on the Summary tab. Moving to Won or Lost triggers the win/loss reason capture modal. Automatic email notifications are sent to the account manager and proposal owner when the status changes.' },
    ],
  },
