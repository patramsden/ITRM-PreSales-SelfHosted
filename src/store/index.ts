...
export const SEED_USERS: User[] = [
  ...
  { id: 'u6', name: 'Laura Singh',  email: 'laura.singh@company.com',  department: 'Finance',    appRole: 'user',  authProvider: 'local' },
  { id: 'u7', name: 'David Okafor', email: 'david.okafor@company.com', department: 'Sales',      appRole: 'user',  authProvider: 'local' },
];
  ...
  updateProposal: (id, updates) => {
    set(s => ({
      proposals: s.proposals.map(p =>
        p.id === id ? { ...p, ...updates, dateModified: today() } : p
      ),
    }));
