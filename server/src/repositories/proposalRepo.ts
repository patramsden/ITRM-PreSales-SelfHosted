...
function toProposal(r: Record<string, unknown>, parts: Part[], phases: ConsultancyPhase[]): Proposal {
  return {
    ...
    clientContactEmail: (r.client_contact_email as string) ?? undefined,
    ...
    proposalType:                 ((r.proposal_type as string) === 'support' ? 'support' : 'project') as Proposal['proposalType'],
    supportContract:              r.support_contract ? JSON.parse(r.support_contract as string) as Proposal['supportContract'] : undefined,
    parts, phases,
  };
}
...
export async function createProposal(p: Proposal): Promise<Proposal> {
  ...
  await query(
    `INSERT INTO proposals (id,project_name,client,account_manager,status,currency,
       ...
       client_contact,client_contact_email,client_address,crm_company_id,milestones,reference,
    ...
    [p.id, p.projectName, p.client, p.accountManager ?? null, p.status, p.currency,
     ...
     p.clientContact ?? null, p.clientContactEmail ?? null, p.clientAddress ?? null,
     p.crmCompanyId ?? null, JSON.stringify(p.milestones ?? []),
     reference,
...
export async function updateProposal(id: string, p: Proposal): Promise<void> {
  await query(
    `UPDATE proposals SET project_name=$2,client=$3,account_manager=$4,status=$5,currency=$6,
       ...
       client_contact=$30,client_contact_email=$31,client_address=$32,crm_company_id=$33,milestones=$34,
       trb_approved_fingerprint=$35,five_k_approved_fingerprint=$36,
